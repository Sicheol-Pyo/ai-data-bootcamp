...
### 최종적으로 일본 여행에 있어 식당 선택 시 구글이나 블로그에서 찾는 것 보다 타베로그를 사용하여 검색하는 것이 좋은 점이 무엇이 있을까?
...
## 마무리 정리
1. 관광지(번화가) 위주의 여행 일정인 경우 타베로그에 나열된 식당을 선택하는 것이 좋은 방법
2. 일본 여행이 다회차인 경우 관광지 보다는 골목골목 안가본 곳을 여행하기에 타베로그로 검색하는 것은 좋은 선택지가 아닐 수 있음.
3. 타베로그에 기재된 맛집의 경우 카드 사용이 가능한 식당이 다수 있고 미리 정보 파악도 가능! 현금 준비 걱정은 No!
4. 일본에서 식당 내 흡연 문화가 아직 있지만 타베로그에서 금연이 필수인 식당도 찾을 수 있고 어린이 동반도 가능 정보도 미리 파악할 수 있어 가족 여행 시에도 타베로그는 좋은 선택지가 될 수 있음.
5. 각 도시별로 유명한 음식이 있기 때문에 도시에 따라 해당 음식점의 검색도 추천.

## 한계점과 추후 분석 정리
1. 예약이 불가능한 식당의 경우 포함되지 않아 누락된 맛집 정보가 있을 수 있음.
2. 현지인들이 많이 사용하는 사이트로 한국인의 리뷰와는 차이가 있을 수 있음.
3. 각 도시별로 60 페이지 총 1200개의 식당 정보가 있어서 추후에는 더 큰 데이터로 비교 필요
4. 각 식당별 시그니처 메뉴 정보와 상세 리뷰 정보는 추가 확인이 필요함.
5. 상위 10% 제외한 메뉴들 중 각 도시별로 특이하게 나타나는 메뉴가 있는지 비교 필요
6. 현재 '예약이 많은 순'으로 분석하였으나 '현지인 예약이 많은 순'과 비교하여 양상이 비슷한지 비교 필요.
7. 각 도시별로 menu를 상세 정보를 취합하여 구체적인 식당 예매 정보를 비교 필요
